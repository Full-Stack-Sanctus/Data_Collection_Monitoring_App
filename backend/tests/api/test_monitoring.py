...
def test_program_performance_endpoint(
    client: TestClient,
) -> None:
    """
    ...
    expected = [
        ProgramPerformance(
            program="digital_skills",
            total_activities=5,
            completed_activities=4,
            target_participants=500,
            actual_participants=450,
    ...
    service.get_program_performance.return_value = (
        expected
    )
    ...
    service.get_program_performance.assert_called_once()
    
    
def test_geographic_performance_endpoint(
    client: TestClient,
) -> None:
    """
    ...
    service.get_geographic_performance.return_value = (
        expected
    )
    ...
    service.get_geographic_performance.assert_called_once()
...
def test_data_quality_summary_endpoint(
    ...
    expected = DataQualitySummary(
        ...
        processing_success_rate=90.0,
    )

    service = Mock()

    service.get_data_quality_summary.return_value = (
        expected
    )
    ...
    service.get_data_quality_summary.assert_called_once()
    
    
def test_data_quality_issue_breakdown_endpoint(
    client: TestClient,
) -> None:
    """
    ...
    service.get_data_quality_issue_breakdown.return_value = (
        expected
    )
    ...
    service.get_data_quality_issue_breakdown.assert_called_once()
    
    
def test_unknown_monitoring_endpoint_returns_404(
    client: TestClient,
) -> None:
    """
