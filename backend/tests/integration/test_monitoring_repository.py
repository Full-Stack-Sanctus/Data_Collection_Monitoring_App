...
location = Location(
    state = f"monitoring-state- {
        identifier
    }",
    lga = f"monitoring-lga- {
        identifier
    }",
    community = f"monitoring-community- {
        identifier
    }",
    latitude = None,
    longitude = None,
...
totals = repository.get_summary_totals(
    db_session
)
...
assert (
    totals["total_target_participants"] == 100
)

assert (
    totals["total_actual_participants"] == 90
)

assert (
    totals["total_male_participants"] == 50
)

assert (
    totals["total_female_participants"] == 40
)

assert (
    totals["total_youth_participants"] == 70
)

assert (
    totals["total_adult_participants"] == 20
)
...
assert (
    totals["total_target_participants"] == 350
)

assert (
    totals["total_actual_participants"] == 270
)

assert (
    totals["total_male_participants"] == 150
)

assert (
    totals["total_female_participants"] == 120
)

assert (
    totals["total_youth_participants"] == 180
)

assert (
    totals["total_adult_participants"] == 90
)
...
assert (
    totals["total_target_participants"] == 0
)

assert (
    totals["total_actual_participants"] == 0
)